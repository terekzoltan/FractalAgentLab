...
| RF-2026-04-27-01 | 2026-04-27 | CV2-A H5 review artifact | Medium | `providers.mock.enabled` can bypass the documented boolean-only enabled-flag strictness because the mock provider path returns before `_is_enabled()` validation | `contract_gap`, `missing_negative_test`, `docs_status_drift` | Track D | open | either validate malformed `providers.mock.enabled` before the mock fast path, or explicitly document and test a mock exemption |
