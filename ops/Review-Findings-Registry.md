...
| RF-2026-04-27-01 | 2026-04-27 | CV2-A H5 review artifact | Medium | `providers.mock.enabled` could bypass the documented boolean-only enabled-flag strictness because the mock provider path returned before `_is_enabled()` validation | `contract_gap`, `missing_negative_test`, `docs_status_drift` | Track D | fixed | fixed by `6fb49cf`; keep boolean-only explicit enabled config covered for every provider, including safe-default `mock` |
| RF-2026-05-03-01 | 2026-05-03 | W5-S1 U5-C step review | Low | Run detail still labeled the trace handoff CTA as `U5-C placeholder` after the U5-C Trace page became active, creating stale transition wording on an otherwise functional path | `surface_inconsistency`, `docs_status_drift` | Track A | fixed | fixed in U5-C closeout; when a placeholder becomes an active surface, include a copy sweep for old placeholder/deferred wording in UI tests and visible labels |
| RF-2026-05-04-01 | 2026-05-04 | W5-S2 U5-D Swarm review | Medium | U5-D workflow catalog generator allowed scalar metadata values broader than the TypeScript catalog model/loader accepted, creating a future generated-catalog contract drift risk if allowlisted metadata becomes numeric or boolean | `contract_gap`, `surface_inconsistency` | Track A | acknowledged; non-blocking | keep generated JSON schema and UI loader types aligned whenever derived UI catalogs allowlist metadata; add schema edge tests if metadata value types expand |
| RF-2026-05-04-02 | 2026-05-04 | W5-S2 U5-D Swarm review | Medium | U5-D relied on App-level integration tests for command builder, packet composer, and workflow catalog loader behavior, leaving adversarial quoting/schema/boundary cases without dedicated unit-level coverage | `missing_negative_test` | Track A | acknowledged; non-blocking | add dedicated builder/composer/loader unit tests when command quoting, packet boundaries, or catalog schema rules grow beyond the current first-pass surface |
| RF-2026-05-04-03 | 2026-05-04 | W5-S2 U5-F Track C wording review | Medium | U5-F delivery wording grouped project-memory sidecars and eval reports under canonical memory/eval truth, overstating Track C memory semantics even though the UI was only read-only inventory | `surface_inconsistency`, `docs_status_drift` | Track A + Meta | fixed | distinguish canonical store truth from derived/source/provenance artifacts whenever UI indexes combine memory, eval, and sidecar surfaces |
