...
from fractal_agent_lab.agents import build_h1_handoff_agent_pack
from fractal_agent_lab.core.models import RunStatus
from fractal_agent_lab.runtime import WorkflowExecutor
from fractal_agent_lab.workflows import H1_HANDOFF_WORKFLOW_ID, build_h1_handoff_workflow_spec


class H1HandoffStepRunnerTests(unittest.TestCase):
    def test_h1_handoff_workflow_runs_end_to_end_on_mock(self) -> None:
        ...
        self.assertEqual(H1_HANDOFF_WORKFLOW_ID, run_state.workflow_id)
        self.assertEqual(["intake", "planner", "critic", "synthesizer"], list(run_state.step_results))

        output_payload = run_state.output_payload or {}
        orchestration = output_payload.get("handoff_orchestration", {})
        self.assertEqual(["intake", "planner", "critic", "synthesizer"], orchestration.get("path"))
        self.assertEqual("synthesizer", orchestration.get("final_step_id"))

        final_output = output_payload.get("final_output", {})
        self.assertIn("clarified_idea", final_output)
        self.assertIn("recommended_mvp_direction", final_output)
