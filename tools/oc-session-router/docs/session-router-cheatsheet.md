...
An ordinary server restart reuses production authority after live stable-
capability validation; the current process is still pinned across each dispatch.
Forward patch updates on the same strict OpenCode `major.minor` line reuse the
proof only with a fresh current capability receipt. Every wider or ambiguous
compatibility change requires a new isolated P0B.
