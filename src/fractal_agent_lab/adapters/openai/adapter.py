...
class OpenAICompatibleAdapter:
    ...
    def execute_step(self, request: AdapterStepRequest) -> AdapterStepResult:
        if not request.model:
            raise RuntimeBoundaryError(
                "OpenAICompatibleAdapter requires an explicit model selection.",
                details={
                    "provider": self.name,
        ...
        response = _parse_json_object(
            ...
            workflow_id=request.workflow_id,
            step_id=request.step_id,
            error_message="OpenAI returned invalid JSON envelope.",
        ...
        output = _parse_json_object(
            ...
            workflow_id=request.workflow_id,
            step_id=request.step_id,
            error_message="OpenAI content is not valid JSON.",
    ...
    def _api_key_env_name(self) -> str:
        ...
        if isinstance(raw_name, str) and raw_name:
            return raw_name
        raise RuntimeBoundaryError(
            "OpenAI provider config 'api_key_env' must be a non-empty string.",
    ...
    def _resolve_api_key(self, api_key_env: str) -> str:
        ...
        if isinstance(api_key, str) and api_key:
            return api_key
        raise RuntimeBoundaryError(
            f"OpenAI API key is missing from env var '{api_key_env}'.",
    ...
    def _timeout_seconds(self) -> float:
        ...
        raise RuntimeBoundaryError(
            "OpenAI provider config 'timeout_seconds' must be a positive number.",
            details={"provider": self.name},
        )
...
def _extract_content_text(
    ...
    *,
    ...
    if not isinstance(choice, Mapping):
        raise _step_failure(
            "OpenAI response is missing a valid first choice.",
            provider=provider,
            workflow_id=workflow_id,
            step_id=step_id,
            fallback_eligible=False,
    ...
    if not isinstance(message, Mapping):
        raise _step_failure(
            "OpenAI response choice is missing message content.",
            provider=provider,
            workflow_id=workflow_id,
            step_id=step_id,
            fallback_eligible=False,
